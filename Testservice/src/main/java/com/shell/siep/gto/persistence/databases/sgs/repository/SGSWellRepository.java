...
import com.shell.siep.gto.persistence.databases.sgs.model.SGSWell;
import com.shell.siep.gto.persistence.repository.GTOWellRepository;

@Repository
public interface SGSWellRepository extends JpaSpecificationExecutor<SGSWell>, GTOWellRepository<SGSWell, String> {

    @QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    List<SGSWell> findAll(@Nullable Specification<SGSWell> spec);

    @Override
    @QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    @Query(value = "select w.wellId from SGSWell w where upper(w.wellId) in :wellIds")
    List<Object[]> findExistingIds(@Param("wellIds") List<String> wellIds);

    @Override
	@QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    @Query(value = "select w.wellId from SGSWell w where upper(w.country.countryName) in :countries")
    List<Object[]> findIdsFromCountries(@Param("countries") List<String> countries);

    @Override
    @QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    @Query(value = "select w.wellId from SGSWell w where upper(w.wellId) in :wellIds and " +
    ...
    @Override
	@QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    @Query(value = "select w.wellId from SGSWell w where upper(w.wellId) like %:wellId%")
    List<Object[]> findWellId(@Param("wellId") String wellId);

    @Override
    @QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    @Query(value = "select 'SGS', w.wellId, w.whLat, w.whLong from SGSWell w where " +
    ...
    @Override
    @QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    @Query(value = "select 'FPC', w.wellId, w.whLat, w.whLong from SGSWell w " +
