...
import com.shell.siep.gto.persistence.databases.dd.model.DDWell;
import com.shell.siep.gto.persistence.repository.GTOWellRepository;

@Repository
public interface DDWellRepository extends JpaSpecificationExecutor<DDWell>, GTOWellRepository<DDWell, String> {

    @QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    List<DDWell> findAll(@Nullable Specification<DDWell> spec);

    @Override
    @QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    @Query(value = "select w.wellId from DDWell w where upper(w.wellId) in :wellIds")
    List<Object[]> findExistingIds(@Param("wellIds") List<String> wellIds);

    @Override
	@QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    @Query(value = "select w.wellId from DDWell w where upper(w.country.countryName) in :countries")
    List<Object[]> findIdsFromCountries(@Param("countries") List<String> countries);

    @Override
    @QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    @Query(value = "select w.wellId from DDWell w where upper(w.wellId) in :wellIds and " +
    ...
    @Override
	@QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    @Query(value = "select w.wellId from DDWell w where upper(w.wellId) like %:wellId%")
    List<Object[]> findWellId(@Param("wellId") String wellId);

    @Override
    @QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    @Query(value = "select 'DD', w.wellId, w.whLat, w.whLong from DDWell w where " +
    ...
    @Override
    @QueryHints(@javax.persistence.QueryHint(name="org.hibernate.fetchSize", value="5000"))
    @Query(value = "select 'DD', w.wellId, w.whLat, w.whLong from DDWell w " +
